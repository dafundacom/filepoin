...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `${env.NEXT_PUBLIC_SITE_URL}/download/app/${slug}`,
    },
    openGraph: {
      title: `Download ${download?.metaTitle ?? download?.title} untuk ${download?.operatingSystem} Versi Terbaru`,
      description: download?.metaDescription ?? download?.excerpt,
      url: `${env.NEXT_PUBLIC_SITE_URL}/download/app/${slug}`,
      images: [
        {
...
export default async function SigleDownloadAppPage({
  ...
  if (language !== locale) {
    if (otherLanguageDownload)
      redirect(
        `/download/app/${otherLanguageDownload.slug}`,
  ...
  return (
    <>
      <ArticleJsonLd
        useAppDir={true}
        url={`${env.NEXT_PUBLIC_SITE_URL}/download/app/${download.slug}`}
        title={`Download ${download?.metaTitle ?? download?.title} untuk ${download?.operatingSystem} Versi Terbaru`}
