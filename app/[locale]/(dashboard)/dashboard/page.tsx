...
export default async function DashboardPage() {
  ...
  const downloads = await api.download.count()
  const downloadFiles = await api.downloadFile.count()
  const downloadComments = await api.downloadComment.count()
    ...
    <>
      ...
      <div className="my-8 grid grid-cols-2 gap-3 md:grid-cols-5">
        <DashboardBox>
          ...
          <DashboardBoxDescription>{t("downloads")}</DashboardBoxDescription>
        </DashboardBox>
        <DashboardBox>
          <DashboardBoxIconWrapper>
            <Icon.UploadFile />
          </DashboardBoxIconWrapper>
          <DashboardBoxCount>{downloadFiles}</DashboardBoxCount>
          <DashboardBoxDescription>
            {t("download_files")}
          </DashboardBoxDescription>
        </DashboardBox>
        <DashboardBox>
          ...
          <DashboardBoxCount>{downloadComments}</DashboardBoxCount>
          <DashboardBoxDescription>{t("comments")}</DashboardBoxDescription>
        </DashboardBox>
      </div>
      <hr />
      <div className="my-8 grid grid-cols-2 gap-3 md:grid-cols-5">
